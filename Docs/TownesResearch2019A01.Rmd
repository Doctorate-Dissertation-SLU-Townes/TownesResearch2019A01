...
# Different transformation required because of data contains 0 and 1 values
  mutate(APPYEARtrfm = log(APPYEAR)) %>%
  mutate(GYEARtrfm = log(GYEAR)) %>%
  mutate(CLAIMStrfm = log(CLAIMS)) %>%
  mutate(CMADEtrfm = log(CMADE)) %>%
  mutate(CRECEIVEtrfm = log(CRECEIVE)) %>%
  mutate(RATIOCITtrfm = log(RATIOCIT)) %>%
  mutate(GENERALtrfm = log(GENERAL)) %>%
  mutate(ORIGINALtrfm = log(ORIGINAL)) %>%
  mutate(FWDAPLAGtrfm = log(FWDAPLAG)) %>%
  mutate(BCKGTLAGtrfm = log(BCKGTLAG)) %>%
  mutate(SELFCTUBtrfm = log(SELFCTUB)) %>%
  mutate(SELFCTLBtrfm = log(SELFCTLB)) %>%
  mutate(SECDUPBDtrfm = log(SECDUPBD)) %>%
  mutate(SECDLWBDtrfm = log(SECDLWBD)) -> Sample90to95C
```
...
CRECEIVEregression <- lm(CRECEIVEtrfm ~ APPYEARtrfm + CAT01 + CAT02 + CAT03 + CAT04 + CAT05 + CAT06 + CMADEtrfm + CLAIMStrfm + ORIGINALtrfm + GENERALtrfm + RATIOCITtrfm + BCKTGLAGtrfm + SELFCTLBtrfm + SECDLWBDtrfm, data = Sample90to95C)
