...
## Clean Data Phase 2
The following code chunk creates additional variables needed for the binary logistic regression, ordinal logistic regression, and multiple regression analyses and removes variables that will not be used.  It first creates a new variable called `CRECbinomial` that converts the `CRECEIVE` variable into a dichotomous variable. It then creates a series of dummy variables for the nominal `CAT` variable to use in multiple regression analysis. Finally, it creates a series of new variables by applying a transformation to the original interval and ratio variables.
```{r clean-data-02}
Sample90to95B %>% 
...
# Plots of Residuals by Independent variables
png(filename = here("Results","ResidualsBaseOrginal.png"))
plot(ORIGINAL, CRECEIVEresid, ylab = "Residuals", xlab = "ORIGINAL", main = "Residuals by ORIGINAL")
abline(0,0)
dev.off()
...
lmtest::dwtest(CRECEIVEregression)
...
# Check that there is no perfect multicollinearity among the variables
...
# Secondary Check of Assumptions
par(mfrow=c(2,2))
gvlma::gvlma(CRECEIVEregression)
plot(CRECEIVEregression)
```
