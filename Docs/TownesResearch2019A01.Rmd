...
Sample90to95A %>%
  ...
  mutate(CAT01 = if(CAT == 1) {CAT02 = 1} else {CAT02 = 0}) %>%
  mutate(CAT01 = if(CAT == 1) {CAT03 = 1} else {CAT03 = 0}) %>%
  mutate(CAT01 = if(CAT == 1) {CAT04 = 1} else {CAT04 = 0}) %>%
  mutate(CAT01 = if(CAT == 1) {CAT05 = 1} else {CAT05 = 0}) %>%
  mutate(CAT01 = if(CAT == 1) {CAT06 = 1} else {CAT06 = 0}) %>%
  ...
  mutate(lnSECDLWBD = log(SECDLWBD)) -> Sample90to95A
```
...
CRECordinal <- polr(as.factor(CRECEIVE) ~ APPYEAR + CAT + CMADE + CLAIMS + ORIGINAL + GENERAL + RATIOCIT, data = Sample90to95A, Hess = TRUE, model = TRUE, method = "logistic")
summary(CRECordinal)
```
...
CRECEIVEregression <- lm(lnCRECEIVE ~ APPYEAR + CAT01 + CAT02 + CAT03 + CAT04 + CAT05 + CAT06 + lnCMADE + lnCLAIMS + lnORIGINAL + lnGENERAL + lnRATIOCIT, data = Sample90to95A)
