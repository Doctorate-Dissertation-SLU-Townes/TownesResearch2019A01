...
## Modify Data 03
The following code chunk creates a new variable called `CRECmdnSplt` using a median split of the `CRECEIVE` values. It then counts the number of observations for each outcome level of the new variable.
```{r modify-data-03}
Sample90to95C %>%
...
The following code chunk uses the new dichotomous variable `CRECmdnSplt` as the dependent variable in a binary logistic regression analysis. It then displays the results. It also calculates the odds ratio, pseudo R-squared values, confidence intervals for the coefficients, and Hosemer-Lemeshow goodness of fit test.
