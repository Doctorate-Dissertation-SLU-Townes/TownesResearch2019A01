...
## Load Raw Data
The following code chunk imports the raw data from the `txt` file.
```{r import-data}
...
The following code chunk calculates the pairwise correlation coefficients for a variables in the sample data using the Pearson product-moment correlation function.
```{r cor-matrix}
correlationMatrix <- cor(Sample90to95, method = "pearson")
correlationVisual <- pairs(Sample90to95, method = "pearson")
```
...
logitCRECEIVE <- glm(CRECbinomial ~ CMADE + CLAIMS + ORIGINAL + GENERAL + RATIOCIT, data = Sample90to95, family = binomial)
summary(logitCRECEIVE)
```

## Save Data
The following code chunk saves the final cleaned data.
```{r save-data}
